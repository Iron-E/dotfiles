  ...
  toXmodmap =
    let
      generator = generators.toKeyValue {
        mkKeyValue =
          let
            _mkKeyValue = generators.mkKeyValueDefault {
              mkValueString =
                let
                  ...
                  typeConverters = {
                    list =
                      v:
                      let
                        converted = map _mkValueString v;
                      in
                      builtins.concatStringsSep " " converted;
          ...
          k:
          let
            key = if builtins.match "^[0-9]+$" k == null then k else "keycode ${k}";
          in
          _mkKeyValue key;
      };
    in
