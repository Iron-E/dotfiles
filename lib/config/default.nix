...
in
{
  ...
  declare =
        ...
        in
        {
          homeConfigurations =
            let
              ...
              mkHomeConfig =
                let
                  deepDefaultHomeConfig = {
                    extraSpecialArgs = mkArgs false;
                  }; # default home config parts which needs to be deeply merged
                in
                username: # `string`
                ...
                in
                home-manager.lib.homeManagerConfiguration mergedHomeConfig;
            in
            mapAttrs' (
              username: homeConfig: nameValuePair "${username}@${hostname}" (mkHomeConfig username homeConfig)
            ) homeConfigsByUsername;

          nixosConfigurations = attrsets.optionalMapByPath [ osConfigKey ] configs (
            osConfig:
            let
    ...
    foldlAttrs
      (
        result: name: value:
        recursiveUpdate result (unpackConfig name value)
      )
      {
        homeConfigurations = { };
        nixosConfigurations = { };
