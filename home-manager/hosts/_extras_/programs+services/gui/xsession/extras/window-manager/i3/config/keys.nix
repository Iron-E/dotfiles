...
let
  inherit (config.xsession.windowManager.i3.config) bars;
  ...
  xsession.windowManager.i3.config = {
    ...
    keybindings =
      let
        ...
        changeTemparature =
          let
            redshift = getExe config.services.redshift.package;
          in
          temp: execInBg "${redshift} -PO ${builtins.toString temp}";

        player =
          let
            playerctl = getExe config.services.playerctld.package;
          in
          cmd: execInBg "${playerctl} ${cmd}";
