	...
	xsession.windowManager.i3.config = {
		...
		let
			...
			let
				...
				popup = multiline /* sh */ ''
					${getExe pkgs.gxmessage} -buttons "" -center -nofocus -font "${fontName} ${fontSize}" -ontop -sticky -timeout 1 -title "Screen Brightness" -***REMOVED***''
				;
			in {
		...
		in {
			...
			### Lock computer
			${lhsModAlt "l"} = exec /* sh */ ''
				export XSECURELOCK_COMPOSITE_OBSCURER=0 && ${getExe pkgs.xsecurelock}'';
