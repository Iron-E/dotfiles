...
let
	...
	addons = mapFn: flip pipe [(map mapFn) toJSON];
...
in {
	...
	programs.librewolf.settings = lib.mapAttrs' (n: lib.nameValuePair "browser.${n}") {
		...
		"policies.runOncePerModification.removeSearchEngines" = toJSON ["Google" "Bing" "Amazon.com" "eBay" "Twitter"];
		...
		"policies.runOncePerModification.extensionsUninstall" =
			addons
			(ext: "${ext}@search.mozilla.org")
			[
				"amazondotcom"
				"bing"
				"ebay"
		...
		"uiCustomization.state" = toJSON (util.attrsets.pair "placements" {
