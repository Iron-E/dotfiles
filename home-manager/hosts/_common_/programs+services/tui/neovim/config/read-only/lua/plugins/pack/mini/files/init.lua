		...
		opts = function(_, o)
			...
			o.content = {
				filter = function(entry)
					return not (
						entry.fs_type == "directory"
						and (entry.name == ".cache" or entry.name == ".git" or entry.name == "node_modules")
					)
				end,
			}
			...
			local function set_dir_to_entry(chdir)
				return function()
					local entry = MiniFiles.get_fs_entry() --- @type fs_entry|nil
					if entry == nil then
						return vim.notify("No FS entry selected", vim.log.levels.INFO)
					end
					local dir = vim.fs.dirname(entry.path)
					vim.notify(
						":" .. (chdir(dir) and "t" or "") .. "cd changed to " .. vim.inspect(dir),
						vim.log.levels.INFO
					)
				end
			end

			local group = vim.api.nvim_create_augroup("config.mini.files", { clear = true })
			vim.api.nvim_create_autocmd("User", {
				desc = "Setup buffer local mappings",
				group = group,
